    ...
    ui(new Ui::Albanien)
{
    ...
    for (int i=0; i<13; i++)
    {
        InfiMonat[i] = Land.InfiMonat(geoID)[i]; //Jeder Element des Ergebnisses der Methode wird in dem
                //entsprechenden Element der Variabel gespeichert
...
void Albanien::on_DatumApply_clicked(QAbstractButton *button)
{
    ...
    QString geoID = "AL"; //geoID um das Land zu indentifieren. Liste von geoIDs in den Ressources-Dateien
